...
def process_boundary(i, boundary, edge_coords, el_set):
        domain = boundary.domain
        results = []
        
        for j, x in enumerate(edge_coords[i]):
...
def apply_results(results, v, n):
    # Extract indices and values from results
    indices = np.concatenate([np.array(res) for res in results], axis=0)
    
    # Create an array of the same length as indices
    values = np.ones(indices.shape[0]) / n

    # Use advanced indexing to update the array `v`
    np.add.at(v, (indices[:, 0], indices[:, 1]), values)
    return None
...
class ConfigurationSpace:
    ...
    def apply_gluing(self,gl):
        ...
        # Extract indices and values from results
        indices = np.array([np.array(res) for res in results])
        ...
        ext = [idx for boundary in gl for e in boundary.domain.edges for idx in boundary.edge_indices]
        ext.sort()
            ...
            for j, x in enumerate(edge_coords[i]):
                ...
                if nzs.size > 0:
                    ...
                    if i == n-1:
                        ...
                        L[nzs] += conjugate_phase * values[:, None] * v[j]
                        pass
                    else:
